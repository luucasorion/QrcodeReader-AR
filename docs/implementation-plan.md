...
> (`Assets/Tests/EditMode/`) and run headless in CI on every PR via GameCI —
> EditMode only, per [ADR 0004](adr/0004-ci-runs-editmode-tests-via-gameci.md). Detection,
> rendering, and memory stay on-device (M5-T6, M6). Keep tested logic decoupled from
> `MonoBehaviour`/MRUK so it stays EditMode-runnable (plan R1).
